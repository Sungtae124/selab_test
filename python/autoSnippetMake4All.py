...
def main():
    ...
    try:
        remote_dir = '/data/bug_db/subjects/defects4j/Lang-39'
        result_dir = f'{remote_dir}/result'
        test_dir = f'{remote_dir}/buggy/src/test/org/apache/commons/lang3'
        ...
        prompt = (
            f"Given the following directories and files from the defect4j dataset Lang-39:\n"
            f"Result directory files: {result_files}\n"
            f"Test directory files: {test_files}\n"
            f"Write a Python script that reads the following relevant files from the server: diff_Lang-39.txt, coverage.json, npe.traces.json, tests.json, and files in the result directory.\n"
